...
use context_course;
use format_designeragenda\options;
...
final class options_test extends \advanced_testcase {
    ...
    public function setUp(): void {
        ...
        $this->course = $this->getDataGenerator()->create_course(['format' => 'designeragenda', 'enablecompletion' => 1]);
        $this->coursecontext = context_course::instance($this->course->id);
    }

    /**
     * Test isjson method find the string is json or not.
     * @covers \format_designeragenda\options::is_json
     */
    public function test_optionisjson(): void {
        ...
        $module = $this->getDataGenerator()->create_module('page', ['course' => $this->course, 'section' => 1,
            ...
            'designeragenda_activityelements' => $elements,
        ]);
        $option = \format_designeragenda\options::get_option($module->cmid, 'activityelements');
        $isjson = \format_designeragenda\options::is_json($option);
        $this->assertTrue($isjson);

        $string = 'Notjson';
        $isjson = \format_designeragenda\options::is_json($string);
    ...
    public function test_moduleelements(): void {
        ...
        $field = $DB->get_field('format_designeragenda_options', 'value', ['name' => 'activityelements', 'cmid' => $module->cmid]);
        $this->assertEquals($elements, json_decode($field, true));

        $option = \format_designeragenda\options::get_option($module->cmid, 'activityelements');
        $this->assertEquals($elements, json_decode($option, true));

        $classes = $PAGE->get_renderer('format_designeragenda')->get_activity_elementclasses((object)['id' => $module->cmid]);
     ...
     * @covers \format_designeragenda\options::is_mod_completed
     */
    public function test_modcompletion(): void {
        ...
        $iscompleted = \format_designeragenda\options::is_mod_completed($cm);
     ...
     * @covers \format_designeragenda\options::is_section_completed
     */
    public function test_sectioncompletion(): void {
        ...
        $iscompleted = \format_designeragenda\options::is_section_completed($section, $this->course, $modinfo, true);
        $this->assertFalse($iscompleted);
