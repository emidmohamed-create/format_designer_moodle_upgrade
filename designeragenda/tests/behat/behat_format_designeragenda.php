 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_format_designeragenda extends behat_base {
    ...
    public function i_edit_the_section_layout($sectionnumber, $layouttype) {
        ...
        if ($CFG->version < 2023092300) {
            ...
            if (get_string_manager()->string_exists($layouttype, 'format_designeragenda')) {
                $strlayout = get_string($layouttype, 'format_designeragenda');
            } else {
                $strlayout = get_string('link', 'format_designeragenda');
            }
            $xpath = $this->execute("behat_course::section_exists", $sectionnumber);
            $xpath .= "/descendant::div[contains(@id, 'section-designeragenda-action')]/descendant::
            ...
            $actionmenu = "Section Layout > ". get_string($layouttype, 'format_designeragenda');
     ...
     *
     * @Given /^I set the designeragenda manual completion$/
     */
     ...
     *
     * @Given /^I set the designeragenda completion expected "(?P<value>(?:[^"]|\\")*)"$/
     * @param string $value
     */
    ...
    public function i_check_the_section_layout($sectionnumber, $layouttype) {
        ...
        $xpath = "//li[@id='section-" . $sectionnumber . "']";
        $xpath .= "/descendant::ul[contains(@class, 'designeragenda-section-content') and contains(@class, '".$layoutclass."')]";
     ...
     *
     * @Given /^I toggle assignment manual completion designeragenda "(?P<acti_id>(?:[^"]|\\")*)" "(?P<acti_type>(?:[^"]|\\")*)"$/
     * @throws DriverException The step is not available when Javascript is disabled
     * @param string $activityname
     * @param string $activityidendifier
     */
    public function i_toggle_assignment_manual_completion_designeragenda($activityname, $activityidendifier) {
        global $CFG;
    ...
    /**
     * Turns block editing mode on.
     * @Given I check the designeragenda section general section
     */
    public function i_check_the_designeragenda_section_general_section() {
        global $CFG;
