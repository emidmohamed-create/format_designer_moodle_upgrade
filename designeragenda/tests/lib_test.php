...
namespace format_designeragenda;
...
final class lib_test extends \advanced_testcase {
    ...
    /**
     * Tests for format_designeragenda::get_section_name method with default section names.
    ...
    public function test_get_section_name(): void {
        ...
        $numsections = 5;
        $course = $generator->create_course(['numsections' => $numsections, 'format' => 'designeragenda'],
            ['createsections' => true]);
    ...
    /**
     * Tests for format_designeragenda::get_section_name method with modified section names.
    ...
    /**
     * Tests for format_designeragenda::get_default_section_name.
    ...
    public function test_get_default_section_name(): void {
        ...
        foreach ($coursesections as $section) {
            if ($section->section == 0) {
                $sectionname = get_string('section0name', 'format_designeragenda');
                $this->assertEquals($sectionname, $courseformat->get_default_section_name($section));
            } else {
                $sectionname = get_string('sectionname', 'format_designeragenda') . ' ' . $section->section;
                $this->assertEquals($sectionname, $courseformat->get_default_section_name($section));
            }
        }
    }
    ...
    public function test_update_inplace_editable(): void {
        ...
        $this->setUser($user);
        $course = $this->getDataGenerator()->create_course(['numsections' => 5, 'format' => 'designeragenda'],
            ['createsections' => true]);
        $section = $DB->get_record('course_sections', ['course' => $course->id, 'section' => 2]);

        // Call webservice without necessary permissions.
        try {
            \core_external::update_inplace_editable('format_designeragenda', 'sectionname', $section->id, 'New section name');
        ...
        $res = \core_external::update_inplace_editable('format_designeragenda', 'sectionname', $section->id, 'New section name');
    ...
    public function test_inplace_editable(): void {
        ...
        // Call callback format_designeragenda_inplace_editable() directly.
        $tmpl = component_callback('format_designeragenda', 'inplace_editable', ['sectionname', $section->id, 'Rename me again']);
    ...
    public function test_default_course_enddate(): void {
        ...
        $params = ['format' => 'designeragenda', 'numsections' => 5, 'startdate' => 1445644800];
     ...
     * @covers ::format_designeragenda_modcontent_trim_char
     * @return void
     */
    public function test_format_designeragenda_modcontent_trim_char(): void {

        $str1 = "Lorem Ipsum is simply dummy text of the printing and typesetting industry.
        Lorem Ipsum has been the industry's standard dummy text ever since the 1500s,
        when an unknown printer took a galley of type and scrambled it to make a type specimen book.";
        $resstr1 = format_designeragenda_modcontent_trim_char($str1, 30);
        ...
        $resstr2 = format_designeragenda_modcontent_trim_char($str2, 30);
        $this->assertEquals(str_word_count($str2), str_word_count($resstr2));
    }

    /**
     * Test desginer format date method.
     * @covers ::format_designeragenda_format_date
     * @return void
     */
    public function test_format_designeragenda_format_date(): void {
        $timestamp = 1642861536;
        $dateformat = format_designeragenda_format_date($timestamp);
    ...
    public function test_kanban_setup(): void {
        global $DB;
        $this->resetAfterTest();
        $record = ['format' => 'designeragenda', 'coursetype' => '0', 'numsections' => 3];
        ...
        $data = ['id' => $course->id, 'coursetype' => DESIGNERAGENDA_TYPE_KANBAN];
     ...
     * @covers ::format_designeragenda_show_staffs_header
     * @return void
     */
    public function test_format_designeragenda_show_staffs_header(): void {
        global $DB;
        $this->resetAfterTest();
        $user = $this->getDataGenerator()->create_user();
        $teacherrole = $DB->get_record('role', ['shortname' => 'editingteacher']);
        $record = ['format' => 'designeragenda'];
        $course = $this->getDataGenerator()->create_course($record);
        $this->getDataGenerator()->enrol_user($user->id, $course->id, $teacherrole->id);
    ...
    public function test_critera_progress(): void {
        ...
        $user = $this->getDataGenerator()->create_user();
        $course = $this->getDataGenerator()->create_course(['format' => 'designeragenda', 'enablecompletion' => 1]);
        $course1 = $this->getDataGenerator()->create_course(['format' => 'designeragenda', 'enablecompletion' => 1]);
        ...
        $criterion = new \completion_criteria_activity();
        $criterion->update_config($criteriadata);
        $result = \format_designeragenda\output\renderer::criteria_progress($course, $user->id);
