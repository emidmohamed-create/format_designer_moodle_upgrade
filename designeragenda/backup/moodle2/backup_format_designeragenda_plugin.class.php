 ...
 */
class backup_format_designeragenda_plugin extends backup_format_plugin {
    ...
    public function define_module_plugin_structure() {
        ...
        $designeragenda = new backup_nested_element('format_designeragenda_options', ['id'], [
            'courseid', 'cmid', 'name', 'value', 'timecreated', 'timemodified',
        ]);

        $designeragenda->set_source_table('format_designeragenda_options',
            ['cmid' => backup::VAR_MODID, 'courseid' => backup::VAR_COURSEID]);

        $files = format_designeragenda\options::get_file_areas('module');
        if ($files) {
            foreach ($files as $file => $component) {
                $designeragenda->annotate_files($component, $file, null);
            }
        }
        $plugin = $this->get_plugin_element(null, $this->get_format_condition(), 'designeragenda');

        return $plugin->add_child($designeragenda);
    }
    ...
    public function define_section_plugin_structure() {
        $formatoptions = new backup_nested_element('designeragenda', ['id'], ['backgroundimage']);

        // Define sources.
        $formatoptions->set_source_table('course_sections', ['id' => backup::VAR_SECTIONID]);

        $files = format_designeragenda\options::get_file_areas('section');
        if ($files) {
            foreach ($files as $file => $component) {
