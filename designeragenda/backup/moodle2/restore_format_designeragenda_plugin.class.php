 ...
 * Specialised restore for Designer course format.
 */
class restore_format_designeragenda_plugin extends restore_format_plugin {
    ...
    public function define_module_plugin_structure() {
        $paths[] = new restore_path_element('format_designeragenda_options', '/module/format_designeragenda_options');
        return $paths;
    }
     ...
     * @param array $data
     */
    public function process_format_designeragenda_options($data) {
        global $DB, $CFG;
        ...
        $DB->insert_record('format_designeragenda_options', $data);
    }
    ...
    public function after_restore_course() {
        ...
        $backupinfo = $this->step->get_task()->get_info();
        if ($backupinfo->original_course_format !== 'designeragenda' || !isset($data['tags']['numsections'])) {
    ...
    protected function after_restore_module() {
        if (!PHPUNIT_TEST) {
            $files = \format_designeragenda\options::get_file_areas();
