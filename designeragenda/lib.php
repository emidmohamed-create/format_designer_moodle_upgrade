 ...
 */
define('DESIGNERAGENDA_SECTION_EXPAND', 1);
/**
 * Collapsible format settings: Collapse all the sections in intial state.
 */
define('DESIGNERAGENDA_SECTION_COLLAPSE', 2);
/**
 * Collapsible format settings: Expand the first the section only in intial state.
 */
define('DESIGNERAGENDA_FIRST_EXPAND', 3);

define('DESIGNERAGENDA_ENABLE_POPUPACTIVITIES', 1);

define('DESIGNERAGENDA_DISABLE_POPUPACTIVITIES', 0);

define('DESIGNERAGENDA_TYPE_NORMAL', 0);

define('DESIGNERAGENDA_TYPE_KANBAN', 1);

define('DESIGNERAGENDA_TYPE_COLLAPSIBLE', 2);

define('DESIGNERAGENDA_TYPE_FLOW', 3);

define('DESIGNERAGENDA_HERO_ZERO_HIDE', 1);

define('DESIGNERAGENDA_HERO_ZERO_VISIBLE', 2);

define('DESIGNERAGENDA_HERO_ACTIVITY_DISABLED', 0);

define('DESIGNERAGENDA_HERO_ACTIVITY_EVERYWHERE', 1);

define('DESIGNERAGENDA_HERO_ACTIVITY_COURSEPAGE', 2);

define('DESIGNERAGENDA_MOD_TEXT_TRIMM', 0);

define('DESIGNERAGENDACOURSEANDSECTIONPAGE', 'courseandsection');

define('DESIGNERAGENDACOURSEPAGE', 'course');

define('DESIGNERAGENDASECTIONPAGE', 'section');

define('DESIGNERAGENDA_PROGRESS_RELEVANTACTIVITIES', 'relevantmods');

define('DESIGNERAGENDA_PROGRESS_ALLACTIVITIES', 'allmods');

define('DESIGNERAGENDA_PROGRESS_SECTIONS', 'sections');

define('DESIGNERAGENDA_CMPIND_DISABLED', 'disabled');

define('DESIGNERAGENDA_CMPIND_BELOWPROGRESS', 'belowcourseprogress');

define('DESIGNERAGENDA_CMPIND_METADATA', 'coursemetadata');

define('DESIGNERAGENDA_PROGRESS_CRITERIA', 'criteria');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_designeragenda extends \core_courseformat\base {
    ...
    public function get_default_section_name($section) {
        if ($section->section == 0) {
            ...
            return get_string('section0name', 'format_designeragenda');
        } else {
    ...
    public function course_header() {
        if (format_designeragenda_has_pro() && class_exists('\local_designer\courseheader')) {
    ...
    public function page_set_course(moodle_page $page) {
        ...
        if ($course->coursedisplay == COURSE_DISPLAY_MULTIPAGE) {
            $page->add_body_class('format-designeragenda-single-section');
        }
        if (format_designeragenda_has_pro()) {
     ...
     */
    public function designeragenda_course_format_options($foreditform = false) {
        global $PAGE, $COURSE;
        ...
        if ($foreditform) {
            ...
            if ($this->designeragenda_completion_enabled()) {
            ...
            $design = \format_designeragenda\options::get_default_options();
    ...
    public function course_format_options($foreditform = false) {
        return $this->designeragenda_course_format_options($foreditform);
    }
    ...
    public static function course_format_options_list($foreditform = false) {
        ...
        if ($courseformatoptions === false) {
            ...
            $courseformatoptions = [
                ...
                'popupactivitiesinfo' => [
                    'default' => get_string('popupactivitiesnotinstalled', 'format_designeragenda'),
                    'type' => PARAM_RAW,
                    'label' => get_string('popupactivities', 'format_designeragenda'),
                ],
                'coursedisplay' => [
            ...
            $courseformatoptions += [
                'courseheader' => [
                    'default' => get_string('courseheader', 'format_designeragenda'),
                    'type' => PARAM_TEXT,
                ],
            ...
            $courseformatoptions += [
                'coursecompletiondateinfo' => [
                    'default' => get_string('completiontrackingmissing', 'format_designeragenda'),
                    'type' => PARAM_TEXT,
                    'label' => new lang_string('coursecompletiondate', 'format_designeragenda'),
                ],
                'timemanagement' => [
                    'default' => '',
                    'type' => PARAM_TEXT,
                ...
                'courseduedateinfo' => [
                    'default' => get_string('timemanagementmissing', 'format_designeragenda'),
                    'type' => PARAM_RAW_TRIMMED,
                    'label' => new lang_string('courseduedate', 'format_designeragenda'),
                ],
                'coursestaff' => [
                    'default' => $teacher->id,
                    'type' => PARAM_TEXT,
            ...
            if (format_designeragenda_has_pro() != 1 ) {
                ...
                if (!empty($userprofilefields)) {
                    foreach ($userprofilefields as $field) {
                        ...
                        $courseformatoptionsedit[$field->inputname] = [
                            ...
                            'help_component' => 'format_designeragenda',
                        ];
                    }
                }
            }
            ...
            $courseformatoptions['courseheroactivityheader'] = [
                'default' => get_string('heroactivity', 'format_designeragenda'),
                'type' => PARAM_TEXT,
            ];
        ...
        if ($foreditform && !isset($courseformatoptions['coursedisplay']['label'])) {
            $courseformatoptionsedit += [
                ...
                'coursedisplay' => [
                    ...
                    'help' => 'coursedisplay',
                    'help_component' => 'moodle',
                    'disabledif' => [['coursetype', 'eq', DESIGNERAGENDA_TYPE_KANBAN]],
                ],

                'accordion' => [
                    'label' => new lang_string('accordion', 'format_designeragenda'),
                    'element_type' => 'select',
                    'element_attributes' => [
                        [
                    ...
                    'disabledif' => [
                        ['coursetype', 'eq', DESIGNERAGENDA_TYPE_KANBAN],
                        ['coursetype', 'eq', 0],
                    ],
                ],

                'initialstate' => [
                    'label' => new lang_string('initialstate', 'format_designeragenda'),
                    'element_type' => 'select',
                    'element_attributes' => [
                        [
                            DESIGNERAGENDA_SECTION_EXPAND => new lang_string('expand', 'format_designeragenda'),
                            DESIGNERAGENDA_SECTION_COLLAPSE => new lang_string('collapse', 'format_designeragenda'),
                            DESIGNERAGENDA_FIRST_EXPAND => new lang_string('firstexpand', 'format_designeragenda'),
                        ],
                    ],
                    'disabledif' => [
                        ['coursetype', 'eq', DESIGNERAGENDA_TYPE_KANBAN],
                ...
                'timemanagement' => [
                    'label' => new lang_string('courseheadertimemanagement', 'format_designeragenda'),
                    'element_type' => 'autocomplete',
                    'element_attributes' => [
                        [
                            'enrolmentstartdate' => new lang_string('enrolmentstartdate', 'format_designeragenda'),
                            'enrolmentenddate' => new lang_string('enrolmentenddate', 'format_designeragenda'),
                            'courseduedate' => new lang_string('courseduedate', 'format_designeragenda'),
                            'coursecompletiondate' => new lang_string('coursecompletiondate', 'format_designeragenda'),
                ...
                'activityprogress' => [
                    'label' => new lang_string('activityprogress', 'format_designeragenda'),
                    'element_type' => 'select',
                    'element_attributes' => [
                        [
                ...
                'coursetype' => [
                    'label' => new lang_string('coursetype', 'format_designeragenda'),
                    'element_type' => 'select',
                    'element_attributes' => [
                        [
                            0 => new lang_string('normal'),
                            DESIGNERAGENDA_TYPE_KANBAN => new lang_string('kanbanboard', 'format_designeragenda'),
                            DESIGNERAGENDA_TYPE_COLLAPSIBLE => new lang_string('collapsiblesections', 'format_designeragenda'),
                            DESIGNERAGENDA_TYPE_FLOW => new lang_string('type_flow', 'format_designeragenda'),
                        ],
                    ],
                    'help' => 'coursetype',
                    'help_component' => 'format_designeragenda',
                ...
                'showanimation' => [
                    'label' => new lang_string('showanimation', 'format_designeragenda'),
                    'element_type' => 'select',
                    'element_attributes' => [
                        [
                            0 => new lang_string('disable'),
                    ...
                    'help_component' => 'format_designeragenda',
                    'disabledif' => [
                        ['coursetype', 'neq', DESIGNERAGENDA_TYPE_FLOW],
                    ],
                ],
                'flowsize' => [
                    'label' => new lang_string('flowsize', 'format_designeragenda'),
                    'element_type' => 'select',
                    'element_attributes' => [
                        [
                            0 => new lang_string('small', 'format_designeragenda'),
                            1 => new lang_string('medium', 'format_designeragenda'),
                            2 => new lang_string('large', 'format_designeragenda'),
                        ],
                    ],
                    'help' => 'flowsize',
                    'help_component' => 'format_designeragenda',
                ...
                'courseheader' => [
                    'label' => new lang_string('courseheader', 'format_designeragenda'),
                    'element_type' => 'header',
                ],
                'listwidth' => [
                    'label' => new lang_string('listwidth', 'format_designeragenda'),
                    'element_type' => 'text',
                    'hideif' => ['coursetype', 'neq', DESIGNERAGENDA_TYPE_KANBAN],
                ],
                'courseindex' => [
                    'label' => new lang_string('courseindex', 'format_designeragenda'),
                    'element_type' => 'select',
                    'element_attributes' => [
                        [
                            0 => new lang_string('show'),
                            self::HIDE_ON_COURSEPAGE => new lang_string('hideoncourses', 'format_designeragenda'),
                            self::HIDE_EVERYWHERE => new lang_string('hideeverywhere', 'format_designeragenda'),
                        ],
                    ],
                    'help' => 'courseindex',
                    'help_component' => 'format_designeragenda',
                ...
                'secondarymenutocourse' => [
                    'label' => new lang_string('strsecondarymenutocourse', 'format_designeragenda'),
                    'element_type' => 'advcheckbox',
            ...
            if (format_designeragenda_popup_installed()) {
                $courseformatoptionsedit['popupactivities'] = [
                    'label' => new lang_string('popupactivities', 'format_designeragenda'),
                    'element_type' => 'select',
                    'element_attributes' => [
                        [
                            DESIGNERAGENDA_DISABLE_POPUPACTIVITIES => new lang_string('disable'),
                            DESIGNERAGENDA_ENABLE_POPUPACTIVITIES => new lang_string('enable'),
                        ],
                    ],
                ...
                $courseformatoptionsedit['addnavigation'] = [
                    'element_type' => 'hidden',
                    'label' => get_string('addnavigation', 'format_designeragenda'),
                ];
                $courseformatoptionsedit['popupactivities'] = [
                    'element_type' => 'hidden',
                    'label' => get_string('popupactivities', 'format_designeragenda'),
            ...
            if (format_designeragenda_timemanagement_installed()) {
            ...
            $courseformatoptionsedit['coursestaff'] = [
                'label' => new lang_string('displayheaderroleusers', 'format_designeragenda'),
                'element_type' => 'autocomplete',
            ...
            $courseformatoptionsedit['courseheroactivityheader'] = [
                'label' => new lang_string('heroactivity', 'format_designeragenda'),
                'element_type' => 'header',
                'help' => 'heroactivity',
                'help_component' => 'format_designeragenda',
            ];
            ...
            $courseformatoptionsedit['sectionzeroactivities'] = [
                'label' => new lang_string('sectionzeroactivities', 'format_designeragenda'),
                'element_type' => 'select',
                'element_attributes' => [
                    [
                        0 => new lang_string('disabled', 'format_designeragenda'),
                        1 => new lang_string('makeherohide', 'format_designeragenda'),
                        2 => new lang_string('makeherovisible', 'format_designeragenda'),
                    ],
                ],
            ...
            $courseformatoptionsedit['heroactivity'] = [
                'label' => new lang_string('showastab', 'format_designeragenda'),
                'element_type' => 'select',
                'element_attributes' => [
                    [
                        0 => new lang_string('disabled', 'format_designeragenda'),
                        1 => new lang_string('everywhere', 'format_designeragenda'),
                        2 => new lang_string('onlycoursepage', 'format_designeragenda'),
                    ],
                ],
                'help' => 'showastab',
                'help_component' => 'format_designeragenda',
     ...
     * @return void
     */
    public function designeragenda_completion_enabled() {
        global $COURSE;
    ...
    public function create_edit_form_elements(&$mform, $forsection = false) {
        ...
        if ($forsection) {
            ...
            $options = $this->designeragenda_course_format_options(true);
        }
    ...
    public function editsection_form($action, $customdata = []) {
        global $CFG;
        require_once($CFG->dirroot. '/course/format/designeragenda/editsection_form.php');
    ...
    public function section_format_options($foreditform = false) {
        ...
        if ($sectionid && $PAGE->pagetype == 'course-editsection') {
            ...
            file_prepare_draft_area($sectionbackdraftid, $coursecontext->id, 'format_designeragenda',
                    'sectiondesignbackground', $sectionid, ['accepted_types' => 'images',
                    'maxfiles' => 1,
                    ],
                );
            file_prepare_draft_area($sectioncompletionbackdraftid, $coursecontext->id, 'format_designeragenda',
    ...
    public static function section_format_options_list($foreditform) {
        ...
        $sectionoptions = [
            'sectiontype' => [
                ...
                'label' => '',
                'element_type' => 'hidden',
                'default' => get_config('format_designeragenda', 'sectiontype'),
            ],
        ];
        $width = [
            0 => '100%',
        ...
        $sectionoptions['sectionlayoutheader'] = [
            'type' => PARAM_TEXT,
            'element_type' => 'header',
            'default' => get_string('sectionlayouts', 'format_designeragenda'),
            'label' => '',
        ];

        $course = course_get_format($PAGE->course)->get_course();
        $settingspage = ($PAGE->course->id == SITEID);
        if ($settingspage || (isset($course->coursetype) && $course->coursetype != DESIGNERAGENDA_TYPE_FLOW)) {
            $lists = [
            ...
            foreach ($lists as $name => $options) {
                ...
                $sectionoptions[$name] = [
                    ...
                    (isset($course->coursetype) && $course->coursetype != DESIGNERAGENDA_TYPE_NORMAL))
                        ? $widthdefaultvalue : $options['default'],
                    'type' => PARAM_INT,
                    'label' => new lang_string($name, 'format_designeragenda'),
                    'element_type' => 'select',
                    'element_attributes' => [
                        $availablewidth,
                    ],
            ...
            $sectionoptions['sectionestimatetime'] = [
                'type' => PARAM_TEXT,
                'element_type' => 'text',
                'label' => get_string('sectionestimatetime', 'format_designeragenda'),
            ];
        }
    ...
    public function duplicate_section(section_info $originalsection): section_info {
        ...
        $fileareasections = [
            'sectiondesignerbackgroundimage' => [
                ...
                'component' => 'format_designeragenda',
            ],
    ...
    public function update_section_format_options($data) {
        ...
        if (empty($data['sectionlayoutheader'])) {
            $data['sectionlayoutheader'] = get_string('sectionlayouts', 'format_designeragenda');
        }
        if (format_designeragenda_has_pro()) {
     ...
     *
     * In case if course format was changed to 'designeragenda', we try to copy options
        ...
        if ($oldcourse !== null) {
            ...
            foreach ($options as $key => $unused) {
                ...
                if ($key == 'courseduedateinfo') {
                    $data[$key] = get_string('timemanagementmissing', 'format_designeragenda');
                }
                if ($key == 'coursecompletiondateinfo') {
                    $data[$key] = get_string('completiontrackingmissing', 'format_designeragenda');
                }
                if ($key == 'popupactivities' && !format_designeragenda_popup_installed()) {
                    $data[$key] = false;
                }
            }

            if (isset($oldcourse['coursetype'])
                && $oldcourse['coursetype'] != DESIGNERAGENDA_TYPE_KANBAN
                && isset($data['coursetype'])
                && $data['coursetype'] == DESIGNERAGENDA_TYPE_KANBAN) {
                    $this->setup_kanban_layouts($oldcourse);
            }
            if (isset($data['coursetype']) && $data['coursetype'] == DESIGNERAGENDA_TYPE_KANBAN) {
    ...
    public function inplace_editable_render_section_name($section, $linkifneeded = true,
        ...
        if (empty($edithint)) {
            $edithint = new lang_string('editsectionname', 'format_designeragenda');
        }
        if (empty($editlabel)) {
            $title = get_section_name($section->course, $section);
            $editlabel = new lang_string('newsectionname', 'format_designeragenda', $title);
        }
        $style = '';
        if (format_designeragenda_has_pro()) {
    ...
    public function get_cm_secondary_title($cm) {
        $secondarytitletype = \format_designeragenda\options::get_option($cm->id, 'secondarytype');
        $title = '';
        if ($secondarytitletype == 'activitytitle') {
            ...
            // Custom title.
            $title = \format_designeragenda\options::get_option($cm->id, 'secondarycustomtitle');
        }
    ...
    public function get_course() {
        ...
        if ($PAGE->pagetype == 'course-edit' && format_designeragenda_has_pro()) {
            // Update the pro fields course values strucuture, Prepare files.
 ...
 * @return inplace_editable
 */
function format_designeragenda_inplace_editable($itemtype, $itemid, $newvalue) {
    global $DB, $CFG;
    require_once($CFG->dirroot . '/course/lib.php');
    if ($itemtype === 'sectionname' || $itemtype === 'sectionnamenl') {
        $section = $DB->get_record_sql(
            'SELECT s.* FROM {course_sections} s JOIN {course} c ON s.course = c.id WHERE s.id = ? AND c.format = ?',
            [$itemid, 'designeragenda'], MUST_EXIST);
 ...
 * @throws dml_exception
 */
function format_designeragenda_format_date(int $timestamp) {
    if ($format = get_config('format_designeragenda', 'dateformat')) {
        $component = strpos($format, 'strf') === 0 ? '' : 'format_designeragenda';
    } else {
 ...
 * @return string
 */
function format_designeragenda_modcontent_trim_char($str, $n = 25) {
 ...
 * @return bool
 */
function format_designeragenda_has_pro() {
    global $CFG;
    static $result;
 ...
 */
function format_designeragenda_get_pro_layouts() {
 ...
 * @return array
 */
function format_designeragenda_get_all_layouts() {
    $layouts = [
        'default' => get_string('link', 'format_designeragenda'),
        'list' => get_string('list', 'format_designeragenda'),
        'cards' => get_string('cards', 'format_designeragenda'),
    ];
    $prolayouts = array_keys(core_component::get_plugin_list('layouts'));
    $prolayouts = (array) get_strings($prolayouts, 'format_designeragenda');
 ...
 */
function format_designeragenda_get_section_background_image($section, $course, $modinfo) {
    if (!empty($section->sectiondesignerbackgroundimage)) {
        $coursecontext = \context_course::instance($course->id);
        $itemid = $section->id;
        $filearea = 'sectiondesignbackground';
        $realtiveactivities = isset($course->calsectionprogress) &&
        ($course->calsectionprogress == DESIGNERAGENDA_PROGRESS_RELEVANTACTIVITIES) ? true : false;
        if (\format_designeragenda\options::is_section_completed($section, $course, $modinfo, true, $realtiveactivities)
            && (isset($section->sectiondesignerusecompletionbg) && $section->sectiondesignerusecompletionbg)) {
            $filearea = 'sectiondesigncompletionbackground';
        }
        $files = get_file_storage()->get_area_files(
            $coursecontext->id, 'format_designeragenda', $filearea,
 ...
 */
function format_designeragenda_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = []) {
    require_login();
    ...
    $fs = get_file_storage();
    $file = $fs->get_file($context->id, 'format_designeragenda', $filearea, $args[0], '/', $args[1]);
    if (!$file) {
        return false;
    }
 ...
 * @return void
 */
function format_designeragenda_include_prosettings($settings) {
    global $CFG, $DB;
    if (format_designeragenda_has_pro() && file_exists($CFG->dirroot.'/local/designer_pro/setting.php')) {
 ...
 */
function format_designeragenda_coursemodule_standard_elements($formwrapper, $mform) {
    global $CFG, $DB;
    $cm = $formwrapper->get_coursemodule();
    $course = $formwrapper->get_course();
    if ($course->format == 'designeragenda') {
        $design = $designadv = \format_designeragenda\options::get_default_options();
        if (isset($cm->id) && $cm->id) {
            $design = \format_designeragenda\options::get_options($cm->id);
        }
        ...
        $mform->addElement('header', 'moduledesign', get_string('activitydesign', 'format_designeragenda'));
        $mform->addElement('html', get_string('activityelementsdisplay', 'format_designeragenda'));
        foreach ($elements as $element => $defalut) {
            // Module background image repeat.
            $name = 'designeragenda_activityelements['.$element.']';
        ...
        // Secondary menu.
        $mform->addElement('html',  get_string('secondarymenu', 'format_designeragenda'));
        $types = [
            'activitytitle' => get_string('stractivitytitle', 'format_designeragenda'),
            'activitytype' => get_string('stractivitytype', 'format_designeragenda'),
            'custom' => get_string('strcustom', 'format_designeragenda'),
        ];
        $mform->addElement('select', 'designeragenda_secondarytype', get_string('secondarymeu_title', 'format_designeragenda'), $types);
        $mform->setType('designeragenda_secondarytype', PARAM_TEXT);
        if (isset($design->secondarytype)) {
            $mform->setDefault('designeragenda_secondarytype', $design->secondarytype);
        }

        $mform->addElement('text', 'designeragenda_secondarycustomtitle', get_string('strcustomtitle', 'format_designeragenda'));
        $mform->setType('designeragenda_secondarycustomtitle', PARAM_TEXT);
        if (isset($design->secondarycustomtitle)) {
            $mform->setDefault('designeragenda_secondarycustomtitle', $design->secondarycustomtitle);
        }
        $mform->hideIf('designeragenda_secondarycustomtitle', 'designeragenda_secondarytype', 'eq', 'activitytitle');
        $mform->hideIf('designeragenda_secondarycustomtitle', 'designeragenda_secondarytype', 'eq', 'activitytype');

        $mform->addElement('advcheckbox', 'designeragenda_customtitleusecourseindex',
            get_string('customnameincourseindex', 'format_designeragenda'));
        $mform->setType('designeragenda_customtitleusecourseindex', PARAM_INT);
        if (isset($design->customtitleusecourseindex)) {
            $mform->setDefault('designeragenda_customtitleusecourseindex', $design->customtitleusecourseindex);
        }
        $mform->hideIf('designeragenda_customtitleusecourseindex', 'designeragenda_secondarytype', 'eq', 'activitytitle');
        $mform->hideIf('designeragenda_customtitleusecourseindex', 'designeragenda_secondarytype', 'eq', 'activitytype');

        $mform->addElement('advcheckbox', 'designeragenda_customtitleuseactivityitem',
            get_string('customnameinactivityitem', 'format_designeragenda'));
        $mform->setType('designeragenda_customtitleuseactivityitem', PARAM_INT);
        if (isset($design->customtitleuseactivityitem)) {
            $mform->setDefault('designeragenda_customtitleuseactivityitem', $design->customtitleuseactivityitem);
        }
        $mform->hideIf('designeragenda_customtitleuseactivityitem', 'designeragenda_secondarytype', 'eq', 'activitytitle');
        $mform->hideIf('designeragenda_customtitleuseactivityitem', 'designeragenda_secondarytype', 'eq', 'activitytype');
        ...
        $mform->addElement('header', 'moduleheroactivity', get_string('heroactivity', 'format_designeragenda'));
        $mform->addElement('select', 'designeragenda_heroactivity', get_string('showastab', 'format_designeragenda'), $tabs);
        $mform->setType('designeragenda_heroactivity', PARAM_INT);
        if (isset($design->heroactivity)) {
            $mform->setDefault('designeragenda_heroactivity', $design->heroactivity);
        }
        $posrange = array_combine(range(-10, 10), range(-10, 10));
        unset($posrange[0]);
        $mform->addElement('select', 'designeragenda_heroactivitypos', get_string('order'), $posrange);
        $mform->setType('designeragenda_heroactivitypos', PARAM_INT);
        $mform->setDefault('designeragenda_heroactivitypos', 0);
        if (isset($design->heroactivitypos)) {
            $mform->setDefault('designeragenda_heroactivitypos', $design->heroactivitypos);
        }
    }
}
 ...
 */
function format_designeragenda_coursemodule_edit_post_actions($data, $course) {
    global $DB;
    $cmid = $data->coursemodule;
    if ($course->format == 'designeragenda') {
        $fields = [
            'designeragenda_activityelements',
            'designeragenda_secondarytype',
            'designeragenda_secondarycustomtitle',
            'designeragenda_customtitleusecourseindex',
            'designeragenda_customtitleuseactivityitem',
            'designeragenda_heroactivity',
            'designeragenda_heroactivitypos',
            'designeragenda_purpose',
        ];
        foreach ($fields as $field) {
            if (!isset($data->$field)) {
                continue;
            }
            $name = str_replace('designeragenda_', '', $field);
            if (isset($data->$field)) {
                ...
                \format_designeragenda\options::insert_option($cmid, $course->id, $name, $value);
            }
        }
    }
    return $data;
 ...
 */
function format_designeragenda_timemanagement_installed() {
    global $DB, $CFG;
 ...
 * @return void
 */
function format_designeragenda_editsetting_style($page) {
    if ($page->user_is_editing()) {
        // Fixed the overlapping issue by make this css rule as important. Moodle CI doesn't allow important.
        $style = '.format-designeragenda .course-content ul.designeragenda .kanban-board-activities li.section:first-child .right .dropdown
 ...
 * @return string|null
 */
function format_designeragenda_get_module_layoutclass($format, $section) {
    $sectiontype = $format->get_section_option($section->id, 'sectiontype') ?: get_config('format_designeragenda', 'sectiontype');
    ...
    if ($format->get_course()->coursetype == DESIGNERAGENDA_TYPE_FLOW) {
        $sectionlayoutclass = 'card';
        $sectiontype = 'cards';
    }

    $prolayouts = format_designeragenda_get_pro_layouts();
 ...
 * @return bool
 */
function format_designeragenda_popup_installed() {
 ...
 * @return bool
 */
function format_designeragenda_course_has_heroactivity($course) {
    global $DB, $PAGE;
    $iscourseheroactivity = ($course->sectionzeroactivities &&
        $course->heroactivity == DESIGNERAGENDA_HERO_ACTIVITY_EVERYWHERE) ? true : false;
    $sql = "SELECT fd.value FROM {format_designeragenda_options} fd
 ...
 * @return bool.
 */
function format_designeragenda_course_has_videotime($course) {
    global $DB;
    $pluginman = \core_plugin_manager::instance();
 ...
 */
function format_designeragenda_extend_navigation_course($navigation, $course, $context) {
    ...
    $isvideotime = format_designeragenda_course_has_videotime($course);
    $jsparams = [
        'courseid' => $course->id,
        'contextid' => $context->id,
        'popupactivities' => $ispopupactivities,
        'isvideotime' => $isvideotime,
        'issubpanel' => format_designeragenda_is_support_subpanel(),
        'sectionreturn' => optional_param('section', 0, PARAM_INT),
    ];
    $PAGE->requires->js_call_amd('format_designeragenda/designeragenda_section', 'init', $jsparams);

    if (format_designeragenda_has_pro()) {
        // Include the designer pro styles.
        $styleurl = \local_designer\courseoptions::create($course)->designeragenda_include_style();
        $PAGE->requires->css($styleurl);
    }

    $isaddsecondary = ($navigation->children->count() <= 1 && $PAGE->context->contextlevel == CONTEXT_MODULE) &&
        (format_designeragenda_course_has_heroactivity($course) || $course->secondarymenutocourse);
    ...
    if ($course->secondarymenutocourse) {
        $secondarymenutocoursecontent .= html_writer::start_tag("li", ["data-key" => 'designeragendacoursehome',
        ...
        get_string('strsecondarymenucourse', 'format_designeragenda'), ['role' => 'menuitem',
            'class' => 'designeragendacoursehome', "tabindex" => "-1" ]);
        $secondarymenutocoursecontent .= html_writer::end_tag("li");

        if (format_designeragenda_has_pro() && $course->prerequisitesbackmain
            ...
            get_string('backtomaincourse', 'format_designeragenda'), ['role' => 'menuitem',
                'class' => 'backmain-course', "tabindex" => "-1", ]);
            $modbacktomain .= html_writer::end_tag("li");
        }
    }
    $sql = "SELECT fd.* FROM
            {format_designeragenda_options} fd
    ...
    $neg = [];
    $pos = [];
    $reports = format_designeragenda_section_zero_tomake_hero($reports, $course);
    ...
    $heroactivityduplicate = get_config("format_designeragenda", "avoidduplicate_heromodentry") == 1 ? true : false;
        ...
        foreach ($reports as $report) {
            if ($report['heroactivity']) {
                ...
                if ($PAGE->context->contextlevel == CONTEXT_MODULE) {
                    if ($report['heroactivity'] == DESIGNERAGENDA_HERO_ACTIVITY_EVERYWHERE) {
                        ...
                        $linkclass = "designeragenda-hero-activity position_$nodepos dropdown-item";
                    ...
                    $node->add_class('designeragenda-hero-activity');
        ...
        require(['jquery', 'core/moremenu'], function($, MenuMore) {
            $(document).ready(function() {
                ...
                function checkPosition(element, pos) {
                    if ($(element.children).hasClass('designeragenda-hero-activity')) {
                        pos += 1;
                ...
                var heroActivity = document.querySelectorAll('.secondary-navigation .designeragenda-hero-activity');
                var i = 0;
                ...
                var designeragendacoursehome = document.querySelectorAll('.moremenu .designeragendacoursehome')[0];
                if (designeragendacoursehome) {
                        designeragendacoursehome.classList.remove('dropdown-item');
                        designeragendacoursehome.classList.add('nav-link');
                        let parent = designeragendacoursehome.parentNode;
                        parent.setAttribute('data-forceintomoremenu', 'false');
 ...
 */
function format_designeragenda_section_zero_tomake_hero($reports, $course) {
    global $PAGE, $DB;
    $course = course_get_format($course->id)->get_course();
            ...
            foreach ($modinfo->sections[0] as $modnumber) {
                if ($DB->record_exists('course_modules', ['deletioninprogress' => 0, 'id' => $modnumber])) {
                    if (isset($reports[$modnumber]) && !$reports[$modnumber]['heroactivity']) {
                        $reports[$modnumber]['heroactivity'] = ($course->heroactivity == DESIGNERAGENDA_HERO_ACTIVITY_COURSEPAGE
 ...
 */
function format_designeragenda_get_coursetypes() {
    $coursetypes = [
        0 => get_string('normal'),
        DESIGNERAGENDA_TYPE_KANBAN => get_string('kanbanboard', 'format_designeragenda'),
        DESIGNERAGENDA_TYPE_COLLAPSIBLE => get_string('collapsiblesections', 'format_designeragenda'),
        DESIGNERAGENDA_TYPE_FLOW => get_string('type_flow', 'format_designeragenda'),
    ];
    return $coursetypes;
}
 ...
 * @return void
 */
function format_designeragenda_fill_custom_values($data, $name, $custom, $csselement) {
 ...
 * @return boolean
 */
function format_designeragenda_is_support_subpanel() {
...
/**
 * Summary of format_designeragenda_get_cache_object
 ...
 */
function format_designeragenda_get_cache_object() {
    return cache::make('format_designeragenda', 'designeragendaoptions');
}
