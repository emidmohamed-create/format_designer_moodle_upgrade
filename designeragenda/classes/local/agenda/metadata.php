...
class metadata {
    ...
    /** @var string Agenda metadata option key. */
    ...
    public static function get_for_cmid(int $cmid): array {
        ...
        $value = \format_designeragenda\options::get_option($cmid, self::OPTION_NAME);
        if ($value === null || $value === '') {
            self::$cache[$cmid] = [];
            return [];
        }
    ...
    public static function preload_for_course(int $courseid, array $cmids): void {
        ...
        $records = $DB->get_records_sql(
            "SELECT cmid, value
               FROM {format_designeragenda_options}
    ...
    public static function save(int $cmid, int $courseid, array $data): void {
        ...
        \format_designeragenda\options::insert_option($cmid, $courseid, self::OPTION_NAME, $value);
    ...
    public static function validate(array $data): array {
        ...
        if (isset($cleaned['modality']) && !in_array($cleaned['modality'], self::MODALITIES, true)) {
            $errors['modality'] = get_string('invalidagendamodality', 'format_designeragenda');
        }
        if (isset($cleaned['pedagogicaltype']) &&
                !in_array($cleaned['pedagogicaltype'], self::PEDAGOGICAL_TYPES, true)) {
            $errors['pedagogicaltype'] = get_string('invalidagendapedagogicaltype', 'format_designeragenda');
        }

        foreach (['timestart', 'timeend'] as $field) {
            if (isset($cleaned[$field]) && !preg_match('/^([01][0-9]|2[0-3]):[0-5][0-9]$/', $cleaned[$field])) {
                $errors[$field] = get_string('invalidagendatime', 'format_designeragenda');
            }
        }
        ...
        if (!isset($errors['timestart'], $errors['timeend']) &&
            ...
            $errors['timeend'] = get_string('agendaendbeforestart', 'format_designeragenda');
        }

        if (isset($cleaned['duration']) &&
                (!ctype_digit((string)$cleaned['duration']) || (int)$cleaned['duration'] > 1440)) {
            $errors['duration'] = get_string('invalidagendaduration', 'format_designeragenda');
        }
        ...
        if (isset($cleaned['meetingurl'])) {
            ...
            if ($url === '' || !in_array(strtolower((string)$scheme), ['http', 'https'], true)) {
                $errors['meetingurl'] = get_string('invalidagendameetingurl', 'format_designeragenda');
            }
        }
