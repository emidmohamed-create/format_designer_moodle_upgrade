...
namespace format_designeragenda\output\courseformat\content\cm;
...
class title extends \core_courseformat\output\local\content\cm\title {
    ...
    protected function get_title_displayvalue(): string {
        ...
        $useactivityitemcustom = \format_designeragenda\options::get_option($mod->id, 'customtitleuseactivityitem');
        ...
        if (format_designeragenda_has_pro()) {
            ...
            if ($textcolor = \format_designeragenda\options::get_option($mod->id, 'textcolor')) {
                $data->moduletextcolor = "color: $textcolor" . ";";
            }
        }

        return $courseoutput->render_from_template(
            'format_designeragenda/courseformat/content/cm/title',
            $data
        );
    }
