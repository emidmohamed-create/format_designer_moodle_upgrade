...
class cm_completion implements renderable, templatable {
    ...
    final public function get_completion_expected(): int {
        if ($duedate = \format_designeragenda\options::timetool_duedate($this->cm)) {
            return $duedate;
        }
    ...
    public function export_for_template(renderer_base $output) {
        ...
        if ($completiondate = $this->get_completion_date()) {
            $data['completiondate'] = format_designeragenda_format_date($completiondate);
        }

        if ($completionexpected = $this->get_completion_expected()) {
            $data['completionexpected'] = format_designeragenda_format_date($completionexpected);
        }
    ...
    private function get_time_ago(int $timestamp): string {
        ...
        $string = [
            'y' => get_string('timeagoyear', 'format_designeragenda'),
            'm' => get_string('timeagomonth', 'format_designeragenda'),
            'w' => get_string('timeagoweek', 'format_designeragenda'),
            'd' => get_string('timeagoday', 'format_designeragenda'),
            'h' => get_string('timeagohour', 'format_designeragenda'),
            'i' => get_string('timeagominute', 'format_designeragenda'),
            's' => get_string('timeagosecond', 'format_designeragenda'),
        ];
        ...
        return $string ? implode(', ', $string) . ' ' . get_string('timeago', 'format_designeragenda')
            : get_string('timeagojustnow', 'format_designeragenda');
    }
}
