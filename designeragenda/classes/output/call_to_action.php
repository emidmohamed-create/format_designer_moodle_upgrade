...
class call_to_action extends cm_completion {
    ...
    final public function get_call_to_action_label(): string {
        ...
        if ($this->is_restricted()) {
            return get_string('calltoactionrestricted', 'format_designeragenda');
        }
        if (!$this->is_tracked_user($USER->id) ||
            !$this->get_completion_info()->is_enabled($this->get_cm())) {
            return get_string('calltoactionview', 'format_designeragenda', $modtype);
        }

        if ($this->get_completion_state() == COMPLETION_INCOMPLETE) {
            return $this->get_completion_data()->viewed ?
                get_string('calltoactioncontinue', 'format_designeragenda', $modtype) :
                get_string('calltoactionstart', 'format_designeragenda', $modtype);
        }
    ...
    public function export_for_template(renderer_base $output) {
        ...
        if (format_designeragenda_has_pro()) {
            $moduledesign = \format_designeragenda\options::get_options($cmid);
            if ($moduledesign) {
