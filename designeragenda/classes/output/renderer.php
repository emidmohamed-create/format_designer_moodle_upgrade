...
use completion_info;
use format_designeragenda\helper;
use html_writer;
use moodle_page;
use moodle_url;
use section_info;
...
use format_designeragenda\output\call_to_action;
use format_designeragenda\output\cm_completion;
...
class renderer extends \core_courseformat\output\section_renderer {
    ...
    public function __construct(moodle_page $page, $target) {
        ...
        // Since format_designeragenda_renderer::section_edit_control_items() only displays the 'Highlight' control
     ...
     *
     * @param \format_designeragenda\output\courseformat\content $widget
    ...
    public function render_content($widget) {
        ...
        if ($course->coursetype == DESIGNERAGENDA_TYPE_KANBAN) {
        ...
        return $this->render_from_template('format_designeragenda/courseformat/content/section', $data);
    }

    /**
     * Render the course module title to add textcolor.
     *
     * @param format_designeragenda\courseformat\content\cm\title $widget
    ...
    public function render_title($widget) {
        ...
        if (format_designeragenda_has_pro()) {
            ...
            if ($textcolor = \format_designeragenda\options::get_option($data->mod->id, 'textcolor')) {
                $data->moduletextcolor = "color: $textcolor" . ";";
            }
        }
        return $this->render_from_template('format_designeragenda/courseformat/content/cm/title', $data);
    }
    ...
    public function completioninfo_icon(\completion_info $completioninfo) {
        ...
        if ($completioninfo->is_enabled() && !$this->page->user_is_editing()
            ...
            $result .= html_writer::tag('div', get_string('yourprogress', 'completion') .
                    $this->output->help_icon('completionicons', 'format_designeragenda'), [
    ...
    public static function get_course_completion_indicator($course) {
        ...
        if ($progress) {
            if (round($progress) == 100) {
                $status = get_string('strcompleted', 'format_designeragenda');
                $class = "completed";
            } else if (round($progress) > 0) {
                $status = get_string('strinprogress', 'format_designeragenda');
                $class = "inprogress";
            }
        } else if (is_enrolled($context, $USER->id)) {
            $status = get_string('strenrolled', 'format_designeragenda');
            $class = "enrolled";
        }
        return [$status, $class];
    }
    ...
    public function course_header_metadata_details(\stdClass $course, bool $dataonly=false) {
        ...
        if (format_designeragenda_has_pro()) {
            ...
            $data += [
                ...
                ($course->completionindicator == DESIGNERAGENDA_CMPIND_METADATA) ? true : false,
            ];
        }

        $courseprogress = self::criteria_progress($course, $USER->id);
        $data['courseprogress'] = ($course->activityprogress) ? $courseprogress : '';
        $data['progresshelpicon'] = $this->output->help_icon('criteriaprogressinfo', 'format_designeragenda');
        ...
        if (format_designeragenda_timemanagement_installed() && function_exists('ltool_timemanagement_cal_course_duedate')) {
        ...
        $html = $this->output->render_from_template('format_designeragenda/course_time_management', $data);
        return $html;
    }
    ...
    public function due_overdue_activities_count(): array {
        global $USER, $DB;
        $cache = format_designeragenda_get_cache_object();
    ...
    public static function get_count_sections_incourse($course) {
        ...
        if (!$cache->get($key)) {
            ...
            foreach ($modinfo->sections as $sectionno => $modnumbers) {
                ...
                if (\format_designeragenda\options::is_vaild_section_completed($section, $course,
    ...
    public static function criteria_progress($course, $userid) {
        ...
        if ($cache->get($cachekey) === false) {
            ...
            } else if ($course->calcourseprogress == DESIGNERAGENDA_PROGRESS_CRITERIA) {
                $isapplycompletioncourses = true;
            }
            ...
            if (format_designeragenda_has_pro()) {
                ...
                if ($course->calcourseprogress == DESIGNERAGENDA_PROGRESS_ALLACTIVITIES) {
                    $count = count($modules);
                    $completionactivities = $modules;
                    $cmidentifier = "id";
                } else if ($course->calcourseprogress == DESIGNERAGENDA_PROGRESS_SECTIONS) {
                ...
                foreach ($completionactivities as $activity) {
                    ...
                    if (isset($modules[$cmid])) {
                        ...
                        $modtooltiplink = html_writer::link($modules[$cmid]->url,
                            get_string('stractivity', 'format_designeragenda') . ": " . $modules[$cmid]->name);
                        if ($data->completionstate == COMPLETION_COMPLETE ||
            ...
            if ($isapplycompletioncourses  && $complteioncourses) {
                foreach ($complteioncourses as $coursecriteria) {
                    ...
                    ['id' => $course->id]), get_string('strcourse', 'format_designeragenda') . ": " . $course->fullname);
            ...
            if (format_designeragenda_has_pro()) {
                $sectiontooltiplink = '';
                if (isset($course->calcourseprogress) && $course->calcourseprogress == DESIGNERAGENDA_PROGRESS_SECTIONS
                    ...
                    foreach ($modinfo->sections as $sectionno => $modnumbers) {
                        ...
                        if ($section->visible) {
                            ...
                            $sectiontooltiplink = html_writer::link($sectionurl,
                                    get_string('strsection', 'format_designeragenda') . ": ". $sectionname);
                            $realtiveactivities = isset($course->calsectionprogress) &&
                                    ($course->calsectionprogress == DESIGNERAGENDA_PROGRESS_RELEVANTACTIVITIES) ? true : false;
                            if (\format_designeragenda\options::is_section_completed($section, $course, $modinfo,
            ...
            if (!empty($completedcriteria)) {
                $completioncriteriahtml = html_writer::start_div('completion-criteria-toolblock designeragenda-criteria-tooltip');
                    $completioncriteriahtml .= html_writer::start_div('head-block');
                        $completioncriteriahtml .= get_string('struppercompleted', 'format_designeragenda');
            ...
            if (!empty($uncompletedcriteria)) {
                $uncompletioncriteriahtml = html_writer::start_div('uncompletion-criteria-toolblock designeragenda-criteria-tooltip');
                $uncompletioncriteriahtml .= html_writer::start_div('head-block');
                    $uncompletioncriteriahtml .= get_string('strtodo', 'format_designeragenda');
    ...
    public function is_template_exists($templatename) {
        try {
            ...
            $templatename = 'format_designeragenda/section_layout_default';
        }
        return $templatename;
    }
    ...
    public function course_type_class($course) {
        ...
        if ($course->coursetype == DESIGNERAGENDA_TYPE_NORMAL && $course->coursedisplay == COURSE_DISPLAY_MULTIPAGE) {
            $class = 'course-type-normal';
        } else if ($course->coursetype == DESIGNERAGENDA_TYPE_COLLAPSIBLE) {
            $class = 'course-type-collapsible';
        } else if ($course->coursetype == DESIGNERAGENDA_TYPE_KANBAN) {
            $class = 'course-type-kanbanboard kanban-board';
        } else if ($course->coursetype == DESIGNERAGENDA_TYPE_FLOW) {
    ...
    public function course_type_sectionclasses($course, $section, $modinfo) {
        ...
        $class = "";
        if ($course->coursetype == DESIGNERAGENDA_TYPE_COLLAPSIBLE) {
    ...
    public function render_section_data(section_info $section, stdClass $course, $onsectionpage,
        ...
        if (format_designeragenda_has_pro() && !$section->uservisible && $section->availableinfo
                && !empty($section->sectioncardredirect)) {
            $sectionurl = $section->sectioncardredirect;
        }
        /** @var format_designeragenda $format */
        $format = course_get_format($course);
        $sectionstyle = '';
        if ($section->section != 0) {
        ...
        $realtiveactivities = isset($course->calsectionprogress) &&
            ($course->calsectionprogress == DESIGNERAGENDA_PROGRESS_RELEVANTACTIVITIES) ? true : false;
        $sectiondata = \format_designeragenda\options::is_section_completed($section, $course, $modinfo, false, $realtiveactivities);
        ...
        if ($course->coursetype == DESIGNERAGENDA_TYPE_FLOW) {
            $sectionlayoutclass = 'card-layout';
            $sectiontype = 'cards';
        }
        $sectioncollapsestatus = '';
        if (isset($course->initialstate) && $course->initialstate == DESIGNERAGENDA_SECTION_COLLAPSE) {
            $sectioncollapsestatus = '';
        } else {
            $sectioncollapsestatus = (isset($course->initialstate) && $course->initialstate == DESIGNERAGENDA_FIRST_EXPAND)
            ? (($section->section == 0) ? 'show' : '') : 'show';
        }
        // Disable the collapsible for kanban board.
        if ( !($course->coursedisplay == 1 && !$onsectionpage)
            && ($course->coursetype == DESIGNERAGENDA_TYPE_COLLAPSIBLE || $course->coursetype == DESIGNERAGENDA_TYPE_FLOW) ) {
        ...
        if ($course->coursetype == DESIGNERAGENDA_TYPE_FLOW && count($modinfo->sections) <= 1) {
        ...
        $sectionstylerules = ($course->coursetype == DESIGNERAGENDA_TYPE_KANBAN)
        ...
        $templatecontext = [
            ...
            'sectioncardcontentdirect' => (format_designeragenda_has_pro() && $course->coursedisplay == COURSE_DISPLAY_MULTIPAGE)
            ...
            'flowcourse' => isset($course->coursetype) && $course->coursetype == DESIGNERAGENDA_TYPE_FLOW ? true : false,
            'maskimage' => (isset($section->sectiondesignermaskimage) && $section->sectiondesignermaskimage) ? true : false,
            'flowsizeclass' => (isset($course->flowsize) && $course->coursetype == DESIGNERAGENDA_TYPE_FLOW &&
            !$this->page->user_is_editing()) ? $this->get_flow_size($course) : '',
        ];
        $zerotohero = $course->sectionzeroactivities;
        if ($zerotohero == DESIGNERAGENDA_HERO_ZERO_HIDE && $section->section == 0 && !$this->page->user_is_editing()) {
            $templatecontext['hidesection'] = true;
        }

        if ($course->coursedisplay == COURSE_DISPLAY_MULTIPAGE && $course->coursetype == DESIGNERAGENDA_TYPE_NORMAL && $sectionheader) {
            $mods = [];
            ...
            foreach ($cmids as $cmid) {
                ...
                if (!$thismod->get_course_module_record()->deletioninprogress) {
                    ...
                    if (format_designeragenda_has_pro() && isset($course->activitydisplaymode)
        ...
        if (format_designeragenda_has_pro() && $showprerequisites) {
            require_once($CFG->dirroot. "/local/designer/lib.php");
        ...
        if ($sectioncontent) {
            $contenttemplatename = 'format_designeragenda/section_content_' . $sectiontype;
    ...
    public function render_course_module($mod, $sectionreturn, $displayoptions = [], $section=null, $cmdata=[]) {
        ...
        if ($course->coursetype == DESIGNERAGENDA_TYPE_FLOW && !$this->page->user_is_editing()) {
            if ((isset($course->showanimation) && $course->showanimation)) {
                ...
                $duration = get_config('format_designeragenda', 'flowanimationduration');
        ...
        if (format_designeragenda_has_pro()) {
            if ($mod->modname == 'videotime' && $dbman->table_exists('videotimeplugin_pro')) {
                if ($videorecord = $DB->get_record('videotimeplugin_pro', ['videotime' => $mod->instance])) {
                    if (isset($videorecord->label_mode) && $videorecord->label_mode == 2) {
                        $useactivityimage = \format_designeragenda\options::get_option($mod->id, 'useactivityimage');
            ...
            $enableactivityimage = \format_designeragenda\options::get_option($mod->id, 'useactivityimage');
        }
        ...
        if (!empty($url) && !$videotime) {
            ...
            $cmtextlength = get_config('format_designeragenda', 'activitydesclength');
            $modcontent = '';
            if (!empty($cmtextcontent)) {
                if ($cmtextlength == DESIGNERAGENDA_MOD_TEXT_TRIMM) {
                    $trimlenght = get_config('format_designeragenda', 'modtrimlength');
                    if (str_word_count($cmtextcontent) >= 23) {
                        $modcontenthtml = '';
                        $modcontenthtml .= html_writer::start_tag('div', ['class' => 'trim-summary']);
                        $modcontenthtml .= format_designeragenda_modcontent_trim_char($cmtextcontent, $trimlenght);
                        ...
                        $modcontenthtml .= " " .\html_writer::link('javascript:void(0)', get_string('less', 'format_designeragenda'),
                        ['class' => 'mod-description-action']);
                        $modcontenthtml .= html_writer::end_tag('div');
        ...
        $modvisits = !empty($modvisits) ? get_string('modvisit', 'format_designeragenda', $modvisits) :
            get_string('notvisit', 'format_designeragenda');
        ...
        $cmlist = [
            ...
            'editagendaurl' => new moodle_url('/course/format/designeragenda/agenda.php', ['cmid' => $mod->id]),
        ];
        if (format_designeragenda_has_pro()) {
            require_once($CFG->dirroot. "/local/designer/lib.php");
    ...
    public function course_index_drawer(course_format $format): ?String {
        if ($format->uses_course_index()) {
            ...
            return $this->render_from_template('format_designeragenda/courseformat/courseindex/drawer', []);
        }
        return '';
    }
    ...
    public function get_activity_elementclasses($mod) {
        ...
        $option  = \format_designeragenda\options::get_option($mod->id, 'activityelements');
        if (!empty($option)) {
    ...
    public function get_cmurl($mod) {
        $options = (format_designeragenda_has_pro()) ? \local_designer\options::get_options($mod->id) : [];
        if ($mod->url) {
            return $mod->url;
    ...
    public function course_section_updated(
        ...
        $output = $this->page->get_renderer('format_designeragenda');
        $sectionclass = $format->get_output_classname('content\\section');
        $sectionobj = new $sectionclass($format, $section);
        return $this->render_from_template('format_designeragenda/section',
    ...
    public function course_section_updated_cm_item(
        ...
        $sectiontype = $format->get_section_option($section->id, 'sectiontype') ?: get_config('format_designeragenda', 'sectiontype');
        $templatename = 'format_designeragenda/cm/module_layout_' . $sectiontype;
