...
class options {
    ...
    public static function get_option(int $cmid, $name) {
        global $DB;
        if ($data = $DB->get_field('format_designeragenda_options', 'value',
            ['cmid' => $cmid, 'name' => $name])) {
            return $data;
        }
        return null;
    ...
    public static function get_options($cmid) {
        ...
        if ($records = $DB->get_records('format_designeragenda_options', ['cmid' => $cmid])) {
    ...
    public static function insert_option(int $cmid, int $courseid, $name, $value) {
        ...
        if ($exitrecord = $DB->get_record('format_designeragenda_options', [
            'cmid' => $cmid, 'courseid' => $courseid, 'name' => $name, ])) {
            $record->id = $exitrecord->id;
            $record->timecreated = $exitrecord->timecreated;
            $DB->update_record('format_designeragenda_options', $record);
        } else {
            $record->timecreated = time();
            $DB->insert_record('format_designeragenda_options', $record);
        }
    }
    ...
    public static function is_mod_completed($mod) {
        $cmcompletion = new \format_designeragenda\output\cm_completion($mod);
    ...
    public static function timetool_duedate($cm) {
        global $USER;
        if (format_designeragenda_timemanagement_installed() && function_exists('ltool_timemanagement_get_mod_user_info')) {
    ...
    public static function get_default_options($issection=false) {
        ...
        if ($design == null) {
            ...
            $formatdesign = (array) get_config('format_designeragenda');
