 ...
 * @return bool result
 */
function xmldb_format_designeragenda_upgrade($oldversion) {
    global $CFG, $DB;
    ...
    if ($oldversion < 2022020301) {
        ...
        $table = new xmldb_table('format_designeragenda_options');
        ...
        upgrade_plugin_savepoint(true, 2022020301, 'format', 'designeragenda');
    }
    ...
    if ($oldversion < 2023040601) {
        ...
        upgrade_plugin_savepoint(true, 2023040601, 'format', 'designeragenda');
    }

    if ($oldversion < 2024073000) {
        $deletesql = <<<EOF
            SELECT fdo.id AS optionid
                FROM {format_designeragenda_options} fdo
                LEFT JOIN {course_modules} cm ON cm.id = fdo.cmid
                WHERE cm.id IS NULL
        EOF;
        $DB->delete_records_subquery('format_designeragenda_options', 'id', 'optionid', $deletesql);
        upgrade_plugin_savepoint(true, 2024073000, 'format', 'designeragenda');
    }

    if ($oldversion < 2026072401) {
        // Agenda metadata uses the dedicated Designer Agenda options table.
