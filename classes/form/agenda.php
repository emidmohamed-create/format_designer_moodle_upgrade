...
namespace format_designer\form;
...
class agenda extends \moodleform {
    ...
    public function definition(): void {
        ...
        $mform->addElement('header', 'agendaheader', get_string('agendainformation', 'format_designer'));
        ...
        foreach (metadata::get_modalities() as $value) {
            $modalities[$value] = get_string('agendamodality_' . $value, 'format_designer');
        }
        $mform->addElement('select', 'modality', get_string('agendamodality', 'format_designer'), $modalities);
        ...
        foreach (metadata::get_pedagogical_types() as $value) {
            $pedagogicaltypes[$value] = get_string('agendapedagogicaltype_' . $value, 'format_designer');
        }
        $mform->addElement('select', 'pedagogicaltype', get_string('agendapedagogicaltype', 'format_designer'),
            $pedagogicaltypes);
        $mform->setType('pedagogicaltype', PARAM_ALPHANUMEXT);
        $mform->setDefault('pedagogicaltype', $current['pedagogicaltype'] ?? '');

        $mform->addElement('text', 'timestart', get_string('agendatimestart', 'format_designer'),
            ['placeholder' => get_string('agendatimestartplaceholder', 'format_designer'), 'maxlength' => 5]);
        $mform->setType('timestart', PARAM_TEXT);
        $mform->setDefault('timestart', $current['timestart'] ?? '');

        $mform->addElement('text', 'timeend', get_string('agendatimeend', 'format_designer'),
            ['placeholder' => get_string('agendatimeendplaceholder', 'format_designer'), 'maxlength' => 5]);
        $mform->setType('timeend', PARAM_TEXT);
        $mform->setDefault('timeend', $current['timeend'] ?? '');

        $mform->addElement('text', 'duration', get_string('agendaduration', 'format_designer'),
            ['placeholder' => get_string('agendadurationplaceholder', 'format_designer'), 'maxlength' => 4]);
        $mform->setType('duration', PARAM_TEXT);
        $mform->setDefault('duration', $current['duration'] ?? '');

        $mform->addElement('text', 'location', get_string('agendalocation', 'format_designer'),
            ['maxlength' => 255]);
        $mform->setType('location', PARAM_TEXT);
        $mform->setDefault('location', $current['location'] ?? '');

        $mform->addElement('url', 'meetingurl', get_string('agendameetingurl', 'format_designer'),
            ['maxlength' => 2048]);
        $mform->setType('meetingurl', PARAM_URL);
        $mform->setDefault('meetingurl', $current['meetingurl'] ?? '');

        $mform->addElement('textarea', 'deliverable', get_string('agendadeliverable', 'format_designer'),
            ['rows' => 3, 'maxlength' => 2048]);
        $mform->setType('deliverable', PARAM_TEXT);
        $mform->setDefault('deliverable', $current['deliverable'] ?? '');

        $mform->addElement('textarea', 'practicalinfo', get_string('agendapracticalinfo', 'format_designer'),
            ['rows' => 3, 'maxlength' => 2048]);
