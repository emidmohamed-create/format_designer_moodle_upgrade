...
class agenda_metadata {
    ...
    public function export_for_template(renderer_base $output): array {
        ...
        if (isset($values['timestart']) || isset($values['timeend'])) {
            ...
            $data['schedulelabel'] = get_string('agendaschedule', 'format_designer');
        }
        if (isset($values['duration'])) {
            $data['hasduration'] = true;
            $data['duration'] = get_string('agendadurationminutes', 'format_designer', (int)$values['duration']);
        }
        if (isset($values['modality'])) {
            $data['hasmodality'] = true;
            $data['modality'] = get_string('agendamodality_' . $values['modality'], 'format_designer');
            $data['modalityicon'] = [
                'onsite' => 'fa-building',
            ...
            $data['modalitylabel'] = get_string('agendamodality', 'format_designer');
        }
        if (isset($values['pedagogicaltype'])) {
            ...
            $data['pedagogicaltype'] = get_string('agendapedagogicaltype_' . $values['pedagogicaltype'],
                'format_designer');
            $data['pedagogicaltypelabel'] = get_string('agendapedagogicaltype', 'format_designer');
        }
        if (isset($values['location'])) {
            $data['haslocation'] = true;
            $data['location'] = $values['location'];
            $data['locationlabel'] = get_string('agendalocation', 'format_designer');
        }
        if (isset($values['meetingurl'])) {
            $data['hasmeetingurl'] = true;
            $data['meetingurl'] = $values['meetingurl'];
            $data['meetingurllabel'] = get_string('agendameetingurl', 'format_designer');
        }
        if (isset($values['deliverable'])) {
            $data['hasdeliverable'] = true;
            $data['deliverable'] = $values['deliverable'];
            $data['deliverablelabel'] = get_string('agendadeliverable', 'format_designer');
        }
        if (isset($values['practicalinfo'])) {
            $data['haspracticalinfo'] = true;
            $data['practicalinfo'] = $values['practicalinfo'];
            $data['practicalinfolabel'] = get_string('agendapracticalinfo', 'format_designer');
        }
